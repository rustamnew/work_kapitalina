/*Step4 вытаскивание описания по клику по кнопке */
let stepPhotoDetailButtons = document.querySelectorAll('.step__photo-mobile-info');
let stepPhotoDetailClose = document.querySelectorAll('.step__photo-content-close');
...
/*Переключение табов в step5*/ 
let stepPaymentButtonCard = document.querySelector('.step__payment-tab-card');
let stepPaymentTabCard = document.querySelector('.step__payment-tab.card');
let stepPaymentInfoCard = document.querySelector('.step__payment-info-card');

let stepPaymentButtonOffice = document.querySelector('.step__payment-tab-office');
let stepPaymentTabOffice = document.querySelector('.step__payment-tab.office');
let stepPaymentInfoOffice = document.querySelector('.step__payment-info-office');

if(stepPaymentButtonCard && stepPaymentButtonOffice) {
	stepPaymentButtonCard.addEventListener('click', (e) => {
		e.preventDefault()
		stepPaymentTabCard.classList.add('active');
		stepPaymentButtonCard.classList.add('active');
		stepPaymentInfoCard.classList.add('active');

		stepPaymentTabOffice.classList.remove('active');
		stepPaymentButtonOffice.classList.remove('active');
		stepPaymentInfoOffice.classList.remove('active');
	})
	stepPaymentButtonOffice.addEventListener('click', (e) => {
		e.preventDefault()
		stepPaymentTabOffice.classList.add('active');
		stepPaymentButtonOffice.classList.add('active');
		stepPaymentInfoOffice.classList.add('active');

		stepPaymentTabCard.classList.remove('active');
		stepPaymentButtonCard.classList.remove('active');
		stepPaymentInfoCard.classList.remove('active');
	})
}
...
///////////////////////////////////////////////////////////////
//Скрипт для вывода превью загруженного изображения (step4)///
//////////////////////////////////////////////////////////////
function handleFileSelect(evt) {
    ...
    reader.onload = (function(theFile) {
        return function(e) {
            ...
            evt.target.nextElementSibling.insertBefore(img, null);
			if ( !navigator.userAgent.match( /iPad|iPhone|Android/i ) ) {
				evt.target.previousElementSibling.innerHTML = 'Изменить';
			}
...
/*Инициализация скрипта для вывода превью */
let photoInputs = document.querySelectorAll('.step__input.file');
